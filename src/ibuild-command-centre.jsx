    ...
    <div style={{display:"flex",height:"100vh",background:_.bg,color:_.ink,fontFamily:"'Inter',-apple-system,BlinkMacSystemFont,sans-serif"}}>
      ...
      <main style={{flex:1,overflowY:"auto",padding:mobile?"72px 16px 88px":"48px 64px 80px"}}>
        ...
        {tab==="schedule"&&<Section key={anim}>
          ...
          {/* Progress + controls — hidden in client view */}
          {!clientView&&<div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:_.s6,paddingBottom:_.s6,borderBottom:`1px solid ${_.line}`,flexWrap:"wrap",gap:_.s4}}>
            <div>
          ...
          {/* CLIENT VIEW — clean presentable table */}
          {clientView&&<>
            ...
            <div style={{marginTop:_.s6,fontSize:12,color:_.faint}}>
              {pName(p)} · Generated {ds()}
            </div>
          </>}

          {/* EDIT VIEW — full controls */}
          {!clientView&&<>
            ...
            {p.milestones.map((ms,i)=>{
              ...
              return(
                <div key={i} draggable={!isEditing} onDragStart={e=>{setDragMs(i);e.dataTransfer.effectAllowed="move";e.currentTarget.style.opacity="0.4"}} onDragEnd={e=>{e.currentTarget.style.opacity="1";if(dragMs!==null&&dragOverMs!==null&&dragMs!==dragOverMs){up(pr=>{const item=pr.milestones.splice(dragMs,1)[0];pr.milestones.splice(dragOverMs,0,item);return pr})}setDragMs(null);setDragOverMs(null)}} onDragOver={e=>{e.preventDefault();e.dataTransfer.dropEffect="move";setDragOverMs(i)}} onDragEnter={e=>e.preventDefault()} style={{display:"grid",gridTemplateColumns:"32px 1fr 60px 120px 120px 80px",gap:8,padding:"10px 0",borderBottom:`1px solid ${_.line}`,alignItems:"center",fontSize:13,background:dragOverMs===i&&dragMs!==null&&dragMs!==i?`${_.ac}08`:isNext?`${_.ac}06`:"transparent",cursor:isEditing?"default":"grab",transition:"background 0.1s"}}>
                  ...
                  {/* Week — editable, with relative offset shifting */}
                  <input type="number" style={{width:48,padding:"3px 6px",background:_.well,border:`1px solid ${_.line}`,borderRadius:_.rXs,color:_.ink,fontSize:12,textAlign:"center",outline:"none",fontWeight:600,fontFamily:"inherit"}} value={ms.wk||0} onChange={e=>{const newWk=parseInt(e.target.value)||0;const delta=newWk-(ms.wk||0);up(pr=>{pr.milestones[i].wk=newWk;if(keepOffsets&&delta!==0){for(let j=i+1;j<pr.milestones.length;j++)pr.milestones[j].wk=Math.max(0,(pr.milestones[j].wk||0)+delta)}return pr})}} />
                  {/* Planned date — with relative offset shifting */}
                  <input type="date" value={ms.planned||""} onChange={e=>{const newDate=e.target.value;up(pr=>{const oldDate=pr.milestones[i].planned;pr.milestones[i].planned=newDate;if(keepOffsets&&oldDate&&newDate){const oldMs=new Date(oldDate).getTime();const newMs2=new Date(newDate).getTime();const deltaMs=newMs2-oldMs;if(deltaMs!==0){for(let j=i+1;j<pr.milestones.length;j++){if(pr.milestones[j].planned){const shifted=new Date(new Date(pr.milestones[j].planned).getTime()+deltaMs);pr.milestones[j].planned=shifted.toISOString().split("T")[0]}}}}return pr})}} style={{padding:"3px 6px",background:_.well,border:`1px solid ${_.line}`,borderRadius:_.rXs,color:_.ink,fontSize:11,outline:"none",cursor:"pointer",fontFamily:"inherit"}} />
                  {/* Completed date */}
