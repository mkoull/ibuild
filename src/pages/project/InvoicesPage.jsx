...
export default function InvoicesPage() {
  ...
  return (
    <Section>
      <h1 style={{ fontSize: mobile ? 28 : 40, fontWeight: 700, letterSpacing: "-0.03em", marginBottom: _.s7 }}>Invoices</h1>
      ...
      <Modal open={deleteIdx !== null} onClose={() => setDeleteIdx(null)} title="Delete Invoice" width={400}>
        <div style={{ fontSize: 14, color: _.body, marginBottom: 24 }}>
          Delete <strong>{deleteIdx !== null && p.invoices[deleteIdx] ? p.invoices[deleteIdx].desc : ""}</strong>?
        </div>
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
          <Button variant="ghost" onClick={() => setDeleteIdx(null)}>Cancel</Button>
          <Button variant="danger" onClick={() => { up(pr => { pr.invoices.splice(deleteIdx, 1); return pr; }); notify("Deleted"); setDeleteIdx(null); }}>Delete</Button>
        </div>
      </Modal>
    </Section>
  );
