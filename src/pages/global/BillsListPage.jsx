...
const STATUS_TABS = ["All", "Draft", "Approved", "Paid", "Void"];
const STATUS_COLORS = { draft: _.muted, approved: _.amber, paid: _.green, void: _.faint };
  ...
  const allBills = useMemo(() => {
    ...
    for (const pr of projects) {
      ...
      for (const bill of pr.supplierBills || []) {
        out.push({ ...bill, projectId: pr.id, projectName: pName(pr, clients) });
      }
    }
  ...
  return (
    <Section>
      <div style={{ marginBottom: _.s5 }}>
        <h1 style={{ fontSize: mobile ? _.fontSize["3xl"] : _.fontSize["4xl"], fontWeight: _.fontWeight.bold, letterSpacing: _.letterSpacing.tight }}>Bills</h1>
        <div style={{ fontSize: _.fontSize.base, color: _.muted, marginTop: _.s1 }}>All supplier bills across projects · Total: {fmt(total)}</div>
      </div>
            ...
            return (
              ...
              >
                <div style={{ minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: _.fontSize.sm, color: _.muted, marginTop: 1 }}>{bill.billNumber || "\u2014"}{bill.date ? ` · ${bill.date}` : ""}</div>
