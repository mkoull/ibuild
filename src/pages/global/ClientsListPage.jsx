...
import { btnPrimary, fmt } from "../../theme/styles.js";
...
export default function ClientsListPage() {
  ...
  const handleNew = () => {
    const c = clientsHook.create({
      ...
      companyName: "",
      notes: "",
      contacts: [mkContact({ role: "Primary" })],
    });
    navigate(`/clients/${c.id}`);
    notify("New client opened");
  };
  ...
  return (
    <Section>
      ...
      {filtered.map(cl => {
        ...
        const primary = (cl.contacts || [])[0];
        const primaryInfo = [primary?.name, primary?.phone || primary?.email].filter(Boolean).join(" · ");
